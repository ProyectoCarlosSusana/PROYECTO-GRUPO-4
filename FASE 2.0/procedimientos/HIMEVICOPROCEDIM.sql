...
--NO VALE  create or replace PROCEDURE LISTARUNTRABAJADOR ( dni_trabajador IN TRABAJADOR.DNI%TYPE, TODO OUT TRABAJADOR%ROWTYPE)
IS
VACIO EXCEPTION;
MAS_ERRORES EXCEPTION;
BEGIN
...
-- NO VALE (VBALE PERO SIN ALL PONIENDO TODOS LOS CAMPOS  create or replace PROCEDURE LISTARUNTRABAJADOR ( dni_trabajador IN TRABAJADOR.DNI%TYPE, TODO OUT sys_refcursor)
IS
VACIO EXCEPTION;
MAS_ERRORES EXCEPTION;
BEGIN
open todo for
SELECT * FROM TRABAJADOR WHERE DNI = dni_trabajador;
EXCEPTION
WHEN NO_DATA_FOUND THEN RAISE VACIO;
WHEN OTHERS THEN RAISE MAS_ERRORES;
END LISTARUNTRABAJADOR;
...
BEGIN
open todo for
SELECT nombre FROM TRABAJADOR WHERE DNI = dni_trabajador;
EXCEPTION
WHEN NO_DATA_FOUND THEN RAISE VACIO;
WHEN OTHERS THEN RAISE MAS_ERRORES;
END LISTARUNTRABAJADOR;
